...
(function(){
  ...
  if(visual){
    ...
    visual.innerHTML='<div class="hero-banner-overlay"><div class="hero-banner-badge">⚓ GLOBAL MARITIME NETWORK</div><strong>2,00,000+ seafarers profiles</strong><span>Seafarers • Jobs • Career Growth</span></div>';
  }
  ...
  if(metrics.length>=3){
    ...
    metrics[0].querySelector('small').textContent='Seafarers profiles';
  ...
  // 2026 indicative market salary guide — USD/month.
  // Actual offers vary by vessel type, flag, company, trade, experience,
  // certificates/CoC and contract or collective agreement. Not guaranteed.
  const salaryByRank={
  ...
  // Market-salary note above the public vacancy grid.
  ...
  document.addEventListener('click',async e=>{
    ...
    if(!job)return;
    let auth=null;
    try{auth=JSON.parse(sessionStorage.getItem('sc_auth')||'null')}catch{}
    // Guest: use the normal SailorCareer registration/login flow.
    if(!auth?.access_token){
      if(window.openAuth)window.openAuth('seafarerRegister');
      return;
    }
    if(auth.role!=='seafarer'){
      if(window.__scToast)window.__scToast('Please use a Seafarer account for vacancy access.');
      return;
    }
    try{
      ...
      // Pro users can see the public vacancy details. These generated marketing vacancies
      // are not database job records, so application submission is not falsely claimed here.
      const isApply=b.classList.contains('public-apply');
      if(window.openModal){
        window.openModal(`<div class="kicker"><span></span> VERIFIED OPPORTUNITY</div><h2>${job.rank}</h2><p><b>Employer details available after application access</b> • ${job.location}</p><div class="dash-grid"><div class="dash-card">Vessel<strong>${job.vessel}</strong></div><div class="dash-card">Contract<strong>${job.contract}</strong></div><div class="dash-card">Market salary<strong>${job.salary}</strong></div></div><h3>${isApply?'Application access':'Vacancy details'}</h3><p>${job.requirements}</p><p class="form-note">${isApply?'This promotional vacancy card is a sample listing. Live employer applications are enabled when the vacancy is connected to the Jobs database.':'You are signed in with Seafarer Pro. Live employer-linked vacancies can be applied to from the Jobs database.'}</p>`);
      }
    }catch(err){if(window.__scToast)window.__scToast(err.message)}
  });

  // Add profile-growth message to the top announcement bar.
  const announce=document.querySelector('.announce');
  if(announce){
    announce.innerHTML='<span>⚓ 2,00,000+ seafarers profiles</span><span>No placement fee is charged to seafarers.</span>';
  }
