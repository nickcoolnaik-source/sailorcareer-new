...
'Bosun','AB','OS','Pumpman','Chief Engineer','2nd Engineer','3rd Engineer',
'4th Engineer','Engine Cadet','Motorman/Oiler','Fitter','ETO','ETR',
'Electrician','Reefer Engineer','Chief Cook','2nd Cook','Steward','Messman',
'Chief Steward','DPO/DP Operator','Rig Manager','Toolpusher','Driller',
'Assistant Driller','Derrickman','Roustabout','Crane Operator','Barge Engineer',
'Fishing Master','Skipper','Deckhand','Marine Surveyor','Marine Superintendent',
'Technical Superintendent','HSQE Officer'
],

vessels:[
'Container Vessel','Bulk Carrier','Tanker','Oil Tanker','Product Tanker',
'Chemical Tanker','LNG Carrier','LPG Carrier','Ro-Ro/Car Carrier',
'General Cargo','Multi-Purpose Vessel','Heavy Lift','Offshore/DP','PSV',
'AHTS','OSV','Drillship','Jack-up Rig','Semi-submersible Rig','FPSO/FSO',
'Cruise/Passenger','Ferry','Yacht','Fishing Vessel','Research Vessel',
'Dredger','Tug/Workboat','Cable Layer','Wind Farm Support',
'Government/Research'
],

sectors:[
'Merchant Shipping','Offshore/Oil & Gas','Cruise & Passenger','Ferries',
'Yachting','Fishing','Research','Government/Public Sector',
'Marine Services','Ship Management'
]
};
...
const authStore={
  ...
  set(v){
    sessionStorage.setItem('sc_auth',JSON.stringify(v));
  },
  clear(){
...
fill('#rankFilter',state.ranks);
fill('#vesselFilter',state.vessels);
fill('#sectorFilter',state.sectors);
...
function renderJobs(list=state.jobs){
  ...
  $('#jobGrid').innerHTML=list.map(j=>`
    <article class="job">
      ...
      <div class="job-top">
        <span class="count">${j.sector}</span>
        <span>✓ Verified</span>
      </div>
      ...
      <p class="requirements">${j.requirements}</p>
...
$('#jobSearch').addEventListener('submit',e=>{
  ...
  const k=$('#keywordFilter').value.toLowerCase();
  ...
  renderJobs(
    state.jobs.filter(j=>
      ...
      (!v||j.vessel===v)&&
      (!s||j.sector===s)&&
      (!k||Object.values(j).join(' ').toLowerCase().includes(k))
    )
  );
});
...
function showSeafarerRegistration(){
  ...
  openModal(`
    <div class="kicker">
      <span></span> SEAFARER REGISTRATION
    </div>

    <h2>Create your Seafarer Profile</h2>

    <p>
      Register as a seafarer to access vacancy details,
      maritime opportunities and application features.
    </p>

    <p>
      Your professional profile will help you manage your
      maritime career on SailorCareer.
    </p>
  ...
  $('#registerForJob').onclick=()=>{
    closeModal();
    openAuth('seafarerRegister');
  };

  $('#loginForJob').onclick=()=>{
    closeModal();
    openAuth('seafarerLogin');
  };
}
...
async function checkSeafarerPro(){
  ...
  try{
    ...
    if(!r.ok){
      throw Error(
        d.error||'Unable to verify subscription'
      );
    }
...
function openProPage(){
  ...
  location.href='/dashboard?upgrade=1';
}
...
window.viewJob=async id=>{
  ...
  */
  ...
  /*
    2. Logged-in user must be seafarer
  */

  if(auth.role!=='seafarer'){

    toast(
      'Please use a Seafarer account to view vacancies.'
    );
  ...
  */
  ...
  if(!isPro){
    ...
    openModal(`
      <div class="kicker">
        <span></span> SEAFARER PRO
      </div>

      <h2>Seafarer Pro Required</h2>

      <p>
        You are signed in as a Seafarer.
        Upgrade to <b>Seafarer Pro — ₹499/year</b>
        to access full vacancy details and
    ...
    $('#viewUpgrade').onclick=openProPage;
  ...
  */
  ...
  openModal(`
    ...
    <div class="dash-grid">
      ...
      <div class="dash-card">
        Vessel
        <strong>${j.vessel}</strong>
      </div>

      <div class="dash-card">
        Contract
        <strong>${j.contract}</strong>
      </div>
...
window.applyJob=async id=>{
  ...
  */
  ...
  if(auth.role!=='seafarer'){
    ...
    toast(
      'Only Seafarer accounts can apply for jobs.'
    );
    ...
    */
    ...
    if(!sd.isPro){
      ...
      openModal(`
        ...
        <h2>Apply with Seafarer Pro</h2>

        <p>
          One-click applications and
          application tracking are included
          with Seafarer Pro.
        </p>
  ...
  openModal(`
    ...
    <div class="form-grid">
      ...
      <button
        ...
        onclick="submitApplication('${j.id}')">
        Submit application
      </button>
    ...
    <p class="form-note">
      Your Seafarer Pro application will be tracked
      from your dashboard.
    </p>
  `);
};
...
window.submitApplication=async id=>{
  ...
  const n=$('#applyName')?.value.trim();
  const e=$('#applyEmail')?.value.trim();
  const p=$('#applyPhone')?.value.trim();

  if(!n||!e||!p){

    toast(
      'Please complete name, email and mobile.'
    );
...
const forms={
  ...
  seafarerRegister:{
    ...
    fields:`
      ...
      <select id="regRank">
        ${state.ranks.map(x=>`<option>${x}</option>`).join('')}
      </select>

      <input
        id="regExperience"
        placeholder="Total sea experience (months)"
      >
  ...
  employerRegister:{
    title:'Employer Portal Registration',
    role:'employer',
    ...
    fields:`
      ...
      <input
        id="coRpsl"
        placeholder="RPSL number / status"
      >
      ...
      <p class="form-note full">
        Employer accounts are created as pending verification.
        Recruitment access remains disabled until admin approval.
      </p>
    `
  }
};
...
function authModalHtml(key){
  ...
  return `
    ...
    <p class="form-note">
      Security verification is required before authentication.
      Email confirmation follows your Supabase Auth settings.
    </p>
  `;
}
...
async function handleAuthSubmit(e){
  ...
  if(payload.action==='login'){
    ...
    payload.nationality=
      $('#regNationality').value.trim();

    payload.rank=
      $('#regRank').value;

    payload.experienceMonths=
      $('#regExperience')
        .value
        .replace(/[^0-9]/g,'')||0;
  ...
  try{
    ...
    if(payload.action==='login'){
      ...
      openModal(`
        ...
        <p class="form-note">
          Your ${roleLabel} account has been
          created successfully and your
          ...
          ?`
            <div
              ...
              <p
                ...
                sign in to complete the
                subscription payment.
                Admin approval will follow
        ...
        <p>
          ...
          <strong>Verify Email</strong> link.
          After verification, return to
          SailorCareer and use
          <strong>
            ${
              f.role==='employer'
              ?'Employer Login'
...
function openPasswordRecovery(accessToken){
  ...
  openModal(`
    ...
    <p class="form-note">
      ...
      sign in again from the appropriate
      SailorCareer portal.
    </p>
  `);
  ...
  $('#recoveryForm').onsubmit=async e=>{
    ...
    try{
      ...
      if(
        ...
        throw Error(
          'Authentication configuration unavailable. Please refresh and try again.'
        );
      }
...
async function handleRecoveryLink(){
  ...
  if(location.search.includes('auth=recovery')){
    ...
    toast(
      'Password recovery link is invalid or expired. Please request a new reset email.'
    );
  }
}
...
function updateHeader(){
  ...
  $$('.head-actions').forEach(x=>{
    ...
    if(!x.dataset.original){
      x.dataset.original=x.innerHTML;
    }
    ...
    async()=>{
      ...
      try{
        ...
        if(
          ...
          toast(
            'Cashfree order created; checkout session returned.'
          );
        }
