...
function showSeafarerRegistration(){
  openModal(`<div class="kicker"><span></span> SEAFARER REGISTRATION</div><h2>Create your Seafarer Profile</h2><p>Register as a seafarer to access vacancy details, maritime opportunities and application features.</p><p>Your professional profile will help you manage your maritime career on SailorCareer.</p><button class="btn primary big" id="registerForJob">Register as Seafarer</button><button class="btn outline big" id="loginForJob">Already registered? Sign in</button>`);
  $('#registerForJob').onclick=()=>{closeModal();openAuth('seafarerRegister')};
  $('#loginForJob').onclick=()=>{closeModal();openAuth('seafarerLogin')};
}
async function checkSeafarerPro(){
  const auth=authStore.get();
  if(!auth?.access_token||auth.role!=='seafarer')return false;
  const r=await fetch('/api/subscription-status',{headers:{Authorization:`Bearer ${auth.access_token}`},cache:'no-store'});
  const d=await r.json();
  if(!r.ok)throw Error(d.error||'Unable to verify subscription');
  return !!d.isPro;
}
function openProPage(){location.href='/dashboard?upgrade=1'}
async function openApplicationForm(j){
  const auth=authStore.get();
  if(!auth?.access_token||auth.role!=='seafarer'){
    showSeafarerRegistration();
    return;
  }
  window.__pendingApplicationJob=j;
  openModal(`<div class="kicker"><span></span> APPLICATION</div><h2>Apply for ${j.rank}</h2><p><b>${j.company||'Participating Employer'}</b> • ${j.location||'—'}</p><div class="dash-grid"><div class="dash-card">Vessel<strong>${j.vessel||'—'}</strong></div><div class="dash-card">Contract<strong>${j.contract||'—'}</strong></div><div class="dash-card">Salary<strong>${j.salary||'Market rate'}</strong></div></div><div class="form-grid"><input id="applyName" placeholder="Full name" value="${auth.full_name||''}" required><input id="applyEmail" type="email" placeholder="Email" value="${auth.email||''}" required><input id="applyPhone" placeholder="Mobile" required><textarea id="applyNote" class="full" placeholder="Short note (optional)"></textarea><button class="btn primary full" onclick="submitApplication()">Submit application</button></div><p class="form-note">Free Seafarers can submit applications. Your latest application status is shown on the dashboard. Full application tracking and My Applications are available with Seafarer Pro.</p>`);
}

window.viewPublicJob=async j=>{
  const auth=authStore.get();
  if(!auth){showSeafarerRegistration();return}
  if(auth.role!=='seafarer'){toast('Please use a Seafarer account to view vacancies.');return}
  const isPro=await checkSeafarerPro();
  if(!isPro){
    openModal(`<div class="kicker"><span></span> VACANCY</div><h2>${j.rank}</h2><p>Basic vacancy information is available. Free Seafarers can apply directly.</p><div class="dash-grid"><div class="dash-card">Vessel<strong>${j.vessel||'—'}</strong></div><div class="dash-card">Location<strong>${j.location||'—'}</strong></div><div class="dash-card">Contract<strong>${j.contract||'—'}</strong></div></div><p>${j.requirements||'Valid CoC / STCW • Relevant sea service • Medical fitness'}</p><button class="btn primary big" id="freeViewApply">Apply Now</button><button class="btn outline big" id="freeViewPro">Upgrade to Pro — ₹499/year</button>`);
    $('#freeViewApply').onclick=()=>openApplicationForm(j);
    $('#freeViewPro').onclick=openProPage;
    return;
  }
  openModal(`<div class="kicker"><span></span> VERIFIED VACANCY</div><h2>${j.rank}</h2><p><b>${j.company||'Participating Employer'}</b> • ${j.location||'—'}</p><div class="dash-grid"><div class="dash-card">Vessel<strong>${j.vessel||'—'}</strong></div><div class="dash-card">Contract<strong>${j.contract||'—'}</strong></div><div class="dash-card">Salary<strong>${j.salary||'Market rate'}</strong></div></div><h3>Requirements</h3><p>${j.requirements||'Valid CoC / STCW • Relevant sea service • Medical fitness'}</p><button class="btn primary big" id="proViewApply">Apply Now</button>`);
  $('#proViewApply').onclick=()=>openApplicationForm(j);
};

window.viewJob=async id=>{
  const j=state.jobs.find(x=>x.id===id); if(!j)return;
  return window.viewPublicJob(j);
};

window.applyPublicJob=async j=>openApplicationForm(j);

window.applyJob=async id=>{
  const j=state.jobs.find(x=>x.id===id); if(!j)return;
  return openApplicationForm(j);
};
...
window.submitApplication=async()=>{
  ...
  const j=window.__pendingApplicationJob;
  if(!a?.access_token||a.role!=='seafarer'||!j){toast('Please sign in as a Seafarer and select a vacancy.');return}
  try{
    const isUuid=v=>/^[0-9a-f-]{36}$/i.test(String(v||''));
    const body={
      job_id:isUuid(j.id)?j.id:null,
      job_reference:isUuid(j.id)?null:String(j.id||''),
      job_rank:j.rank||'',
      job_vessel:j.vessel||'',
      job_company:j.company||'Participating Employer',
      job_location:j.location||'',
      job_contract:j.contract||'',
      job_salary:j.salary||'',
      note:$('#applyNote')?.value||''
    };
    const r=await fetch('/api/applications',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${a.access_token}`},body:JSON.stringify(body)});
    const d=await r.json();
    if(!r.ok)throw Error(d.error||'Unable to submit application');
    toast('Application submitted successfully. Check your dashboard for the latest status.');
    closeModal();
    window.__pendingApplicationJob=null;
